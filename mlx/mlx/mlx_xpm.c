...
char	*mlx_int_static_line(char **xpm_d,int *pos,int size)
{
  static char	*copy = 0;
  static int	len = 0;
  int		len2;
...
void	mlx_int_xpm_set_pixel(mlx_img_list_t *img, char *d, int opp, int col, int x)
{
  /*
  int	dec;
...
void	*mlx_int_parse_xpm(mlx_ptr_t *xvar,void *info,int info_size,char *(*f)())
{
  ...
  while (i--)
	...
	{
	  if (!(clip_d = malloc(4*width*height)) ||   // ok, nice size ..
	      !(clip_img = XCreateImage(xvar->display, xvar->visual,
					1, XYPixmap, 0, clip_d,
					width, height, 8, (width+7)/8)) )
	    RETURN;
	  memset(clip_d, 0xFF, 4*width*height);
	}
      */
      if (method)
  ...
  while (i--)
      ...
      while (x<width)
	{
	  ...
	  if (col==-1)
	    col = 0xFF000000;
	  mlx_int_xpm_set_pixel(img, d, opp, col, x);
	  x ++;
	}
      //      d += img->size_line;
      d += img->width*4;
...
void	*mlx_xpm_to_image(mlx_ptr_t *xvar,char **xpm_d,int *width,int *height)
{
  mlx_img_list_t	*img;

  if ((img = mlx_int_parse_xpm(xvar,xpm_d,0,mlx_int_static_line)))
    {
      *width = img->width;
      *height = img->height;
    }
