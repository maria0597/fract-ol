...
{
  ((mlx_ptr_t *)info)->loop_hook(((mlx_ptr_t *)info)->loop_hook_d);
}
...
void *mlx_init()
{
  ...
  while (i < 4*(FONT_WIDTH+2)*95*FONT_HEIGHT)
    {
      new_mlx->font->buffer[i+0] = font_atlas.pixel_d[i+2];
      new_mlx->font->buffer[i+1] = font_atlas.pixel_d[i+1];
      new_mlx->font->buffer[i+2] = font_atlas.pixel_d[i+0];
      ((unsigned char *)new_mlx->font->buffer)[i+3] = 0xFF-font_atlas.pixel_d[i+3];
      i += 4;
    }
